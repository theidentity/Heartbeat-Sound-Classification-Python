...
class AudioClassifer_NN(object):
	...
	def __init__(self,folds=4):
		...
		self.name = 'Audio_conv_manual_feat'
		self.folds = folds
	...
	def get_data(self):
		...
		while True:
			...
			for train_idx,test_idx in folds.split(X=features,y=np.argmax(labels,axis=1)):
				...
				train_X = train_X.reshape((-1,self.input_cols,1))
				test_X = test_X.reshape((-1,self.input_cols,1))

				yield (train_X,train_y),(test_X,test_y)

	def get_model(self):
		model = Sequential()
		model.add(Conv1D(128,3,activation='relu',input_shape=(self.input_cols,self.channels)))
...
if __name__ == '__main__':
	...
	clf.train_on_folds(lr=1e-6,epochs=1000)
